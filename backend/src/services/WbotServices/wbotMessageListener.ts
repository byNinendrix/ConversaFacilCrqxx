...
// Normaliza qualquer JID (incluindo LID) para o formato padrão s.whatsapp.net
const normalizeJid = (jid?: string | null): string | undefined => {
  if (!jid) return undefined;
  try {
    return jidNormalizedUser(jid); // já lida com @lid → @s.whatsapp.net
  } catch {
    return jid;
  }
};

// Extrai apenas o número (sem @, sem domínio) de um JID
const extractNumberFromJid = (jid?: string | null): string => {
...
import FlowRuntimeService from "../FlowServices/FlowRuntimeService";
import ServiceSchedulingRuntimeService from "../SchedulingServices/ServiceSchedulingRuntimeService";
...
export const getQuotedMessage = (msg: proto.IWebMessageInfo): any => {
  ...
  return extractMessageContent(
    body[Object.keys(body).values().next().value]
  );
};
...
// 👇 Helper para sempre pegar o JID “certo”: se tiver remoteJidAlt, usa ele.
const getMainJid = (msg: any): string => {
  const key: any = msg.key || {};
  const baseJid = key.remoteJidAlt || key.remoteJid; // PRIORIDADE: Alt (número real)
  return jidNormalizedUser(baseJid);
};

const getSenderMessage = (msg: any, wbot: Session): string => {
  ...
  const senderId =
    msg["participant"] ||
    key["participant"] ||
    key.remoteJidAlt || // se vier, é o número “real”
    key.remoteJid;
...
const downloadMedia = async (msg: proto.IWebMessageInfo) => {
  let buffer;
  try {
    // Baileys espera WAMessage, então fazemos um cast interno
...
${JSON.stringify(msg?.message)}`);
      ...
      Sentry.captureException(
        new Error("Novo Tipo de Mensagem em isValidMsg")
      );
    }
...
const verifyQueue = async (
  ...
  if (queues.length === 1) {
    ...
    const dedupeKey = `greet:${companyId}:${contact.id}`;
    const ttl = Number(process.env.GREETING_TTL_SECONDS || 86400);

    const already = await cacheLayer.get(dedupeKey);
    if (!already) {
      await cacheLayer.set(dedupeKey, "1", "EX", ttl);

      const minDelay = Number(process.env.GREETING_DELAY_MIN_MS || 5000);
      const maxDelay = Number(process.env.GREETING_DELAY_MAX_MS || 15000);
      const randomDelay =
        Math.floor(Math.random() * (maxDelay - minDelay + 1)) + minDelay;

      console.log(
        `⏳ Aguardando ${randomDelay}ms antes de enviar saudação...`
      );

      setTimeout(async () => {
        try {
          // 1º Saudação da conexão (WhatsApp)
          ...
          // 2º Anexo da conexão (WhatsApp)
          if (greetingMediaPath && greetingMediaPath !== "") {
            try {
              const filePath = path.resolve("public", greetingMediaPath);
              const optionsMsg = await getMessageOptions(
                greetingMediaName,
                filePath
              );
              await wbot.sendMessage(contactNumber, { ...optionsMsg });
            } catch (mediaErr) {
              console.error("❌ Erro ao enviar anexo da conexão:", mediaErr);
            }
          }

          // 3º Saudação da fila
          if (firstQueue?.greetingMessage && firstQueue.greetingMessage.length > 0) {
            await wbot.sendMessage(contactNumber, {
          ...
          // 4º Anexo da fila
          if (firstQueue?.mediaPath && firstQueue.mediaPath !== "") {
            try {
              ...
              console.error("❌ Erro ao enviar anexo da fila:", mediaErr);
            }
          }

          console.log("✅ Saudação (conexão + fila) enviada com sucesso!");
        } catch (err) {
          console.error("❌ Erro ao enviar saudação:", err);
          try {
            await cacheLayer.del(dedupeKey);
          } catch {}
        }
      }, randomDelay);
    } else {
      console.log(
        `⚠️ Saudação já enviada (TTL ativo) para ${contact.number}`
      );
    }
    ...
    if (
      ...
      if (greetingMediaPath && greetingMediaPath !== "") {
        try {
          ...
          const mediaSentMsg = await wbot.sendMessage(
            `${contact.number}@${
              ticket.isGroup ? "g.us" : "s.whatsapp.net"
            }`,
            { ...optionsMsg }
          );
          await verifyMediaMessage(mediaSentMsg, ticket, ticket.contact);
        } catch (mediaErr) {
          console.error("❌ Erro ao enviar anexo de saudação:", mediaErr);
        }
      }
  ...
  if (!isNil(queue) && isNil(ticket.queueOptionId)) {
    ...
    const botButton = async () => {
      ...
      buttons.push({
        buttonId: `#`,
        buttonText: {
          displayText: "Menu inicial *[ 0 ]* Menu anterior"
        },
        type: 4
      });
    ...
    const currentOption = await QueueOption.findByPk(
  ...
  try {
    ...
    if (isGroup) {
      groupContact = await wbotMutex.runExclusive(async () => {
        ...
        if (!result) {
          const groupMetadata = await wbot.groupMetadata(
            msg.key.remoteJid
          );
    ...
    if (
      ...
      formatBody(whatsapp.complationMessage, contact)
        .trim()
        .toLowerCase() === lastMessage?.body.trim().toLowerCase()
    ) {
      return;
    }
    ...
    if (isGroup || (contact as any)["disableBot"]) {
      return;
    }
    ...
    if (!msg.key.fromMe && !ticket.userId) {
      const handledByFlow = await FlowRuntimeService.tryHandleInbound({
        companyId,
        ticket,
        inputText: bodyMessage || ""
      });
      ...
      const handledByScheduling =
        await ServiceSchedulingRuntimeService.tryHandleInbound({
          companyId,
          ticket,
          inputText: bodyMessage || ""
        });

      if (handledByScheduling) {
        return;
      }
    }
    ...
    if (
      ...
      if (
        lastMessage &&
        lastMessage.body.includes(whatsapp.greetingMessage)
      ) {
        return;
      }
      ...
      if (whatsapp.greetingMessage) {
        ...
        const debouncedSentMessage = debounce(
          async () => {
            ...
            if (whatsapp.greetingMediaPath && whatsapp.greetingMediaPath !== "") {
              try {
...
const verifyCampaignMessageAndCloseTicket = async (
  ...
  const isCampaign = body ? /\u200c/.test(body) : false;
  ...
  try {
    ...
    wbot.ev.on("messages.update", (messageUpdate: WAMessageUpdate[]) => {
      ...
      messageUpdate.forEach(async (message: WAMessageUpdate) => {
        ...
        handleMsgAck(message, (message.update as any)["status"]);
      });
    });
    ...
    Sentry.captureException(error);
    logger.error(
      `Error handling wbot message listener. Err: ${error}`
    );
  }
};

export { handleMessage, wbotMessageListener, getTypeMessage };
