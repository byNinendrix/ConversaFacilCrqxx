...
const typebotListener = async ({
  ...
  async function createSession(
    ...
    try {
      ...
      const request = await axios.request(config as any);
      return request.data;
    } catch (err) {
  ...
  try {
    ...
    dataLimite.setMinutes(
      dataLimite.getMinutes() - Number(typebotExpires)
    );
    ...
    if (
      ...
      if (dataStart?.messages.length === 0 || dataStart === undefined) {
        ...
        requestContinue = await axios.request(config as any);
